...
class Program
{
    ...
    {
        var queryLines = await File.ReadAllLinesAsync("query.txt");
        ...
        while (true)
        {
            ...
            {
                var queryParams = ParseQueryParams(line);

                for (int i = 0; i < parallelRequests; i++)
                {
                    tasks.Add(SendRequest(queryParams));
                }
            }
    ...
    private static NameValueCollection ParseQueryParams(string query)
    {
